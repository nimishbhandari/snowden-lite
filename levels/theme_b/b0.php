...
$sql2 = "SELECT * FROM coder WHERE id=2";
...
if (isset($_POST['ans'])) {
    $ans = md5($_POST['ans']);
    $sql = "SELECT * FROM coder WHERE id=2 AND ans0='$ans'";
    $result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
    $count = mysqli_num_rows($result);
    if ($count == 1) {
        $sql_solver = "UPDATE solver SET thm_b='$thm_b'+1, score='$score'+1 WHERE email='$email'";
        $sql_coder = "UPDATE coder SET score='$c_score'-1 WHERE id=2";
