...
$sql2 = "SELECT * FROM coder WHERE id=10";
...
if (isset($_POST['ans'])) {
    $ans = md5($_POST['ans']);
    $sql = "SELECT * FROM coder WHERE id=10 AND ans5='$ans'";
    $result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
    $count = mysqli_num_rows($result);
    if ($count == 1) {
        $sql_solver = "UPDATE solver SET thm_j='$thm_j'+1, score='$score'+1 WHERE email='$email'";
        $sql_coder = "UPDATE coder SET score='$c_score'-1 WHERE id=10";
